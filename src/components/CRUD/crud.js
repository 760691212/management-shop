...
function CRUD(options) {
  ...
  const methods = {
    ...
    refresh() {
      ...
      if (crud.url) {
        return new Promise((resolve, reject) => {
          ...
          // 请求数据
          initData(crud.url, params, crud.method).then(data => {
    ...
    toAdd(params) {
      crud.editable = true;
      crud.resetForm()
      if (!(callVmHook(crud, CRUD.HOOK.beforeToAdd, crud.form, params) && callVmHook(crud, CRUD.HOOK.beforeToCU, crud.form, params))) {
        return
      }
      crud.status.add = CRUD.STATUS.PREPARED
      callVmHook(crud, CRUD.HOOK.afterToAdd, crud.form, params)
      callVmHook(crud, CRUD.HOOK.afterToCU, crud.form, params)
    ...
    toEdit(data, params) {
      ...
      if (!(callVmHook(crud, CRUD.HOOK.beforeToEdit, crud.form, params) && callVmHook(crud, CRUD.HOOK.beforeToCU, crud.form, params))) {
        return
      }
      crud.status.edit = CRUD.STATUS.PREPARED
      crud.getDataStatus(crud.getDataId(data)).edit = CRUD.STATUS.PREPARED
      callVmHook(crud, CRUD.HOOK.afterToEdit, crud.form, params)
      callVmHook(crud, CRUD.HOOK.afterToCU, crud.form, params)
      if (crud.findVM('form').$refs['form']) {
        crud.findVM('form').$refs['form'].clearValidate()
      }
    ...
    doEdit(callback) {
      ...
      crud.crudMethod.edit(crud.form).then(() => {
        let oldForm = JSON.parse(JSON.stringify(crud.form));
        crud.status.edit = CRUD.STATUS.NORMAL
        crud.getDataStatus(crud.getDataId(crud.form)).edit = CRUD.STATUS.NORMAL
        crud.editSuccessNotify()
        crud.resetForm()
        callVmHook(crud, CRUD.HOOK.afterSubmit, oldForm)
        crud.refresh()
    ...
    resetForm(data) {
      ...
      for (const k in crudFrom) {
        if (crudFrom.hasOwnProperty(k)) delete crudFrom[k];
      }
...
function callVmHookWithResult(crud, hook) {
  if (crud.debug) {
    console.log('callVmHook: ' + hook)
  }
  const tagHook = crud.tag ? hook + '$' + crud.tag : null
  ...
  vmSet.forEach(vm => {
    if (vm[hook]) {
      ret = vm[hook].apply(vm, nargs)
    }
    if (tagHook && vm[tagHook]) {
      ret = vm[tagHook].apply(vm, nargs)
    }
  })
  return ret
}
